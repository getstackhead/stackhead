...
if [[ $INPUT_CLI != '' ]]; then
	...
	sed -e "s/\${ipaddress}/${INPUT_IPADDRESS}/" -e "s/\${dns}/${INPUT_DNS}/" -e "s/\${dns_cloudflare_api_token}/${INPUT_DNS_CLOUDFLARE_APITOKEN}/" -e "s/\${webserver}/${INPUT_WEBSERVER}/" -e "s/\${container}/${INPUT_CONTAINER}/" -e "s/\${application}/container/" -e "s/\${plugins}/${INPUT_PLUGINS}/" "${GITHUB_ACTION_PATH}/inventory.dist.yml" >"${INVENTORY_PATH}"

	CI=1 ansible-playbook "${STACKHEAD_CLONE_LOCATION}/ansible/playbooks/application-deploy.yml" -i "${INVENTORY_PATH}" -vv
fi
