...
sed -e "s/\${domain}/${INPUT_DOMAIN}/" -e "s/\${domain2}/${INPUT_DOMAIN2}/" "${GITHUB_ACTION_PATH}/project-definition.dist.yml" >"${GITHUB_ACTION_PATH}/projects/container.yml"

if [[ $INPUT_CLI != '' ]]; then
	${INPUT_CLI_BIN_PATH} project deploy "${GITHUB_ACTION_PATH}/projects/container.yml" "${INPUT_IPADDRESS}" -c "/tmp/.stackhead-cli.yml" -v
else
