...
func InstallApt() error {
	...
	fileContents, err := xfs.ReadFile("pkging:///templates/modules/proxy/caddy/Caddyfile.tmpl")
	tmpl, err := template.New("caddy").Funcs(sprig.TxtFuncMap()).Parse(fileContents)
	if err != nil {
		return err
	}
	...
	err = xfs.WriteFile("ssh:///etc/caddy/Caddyfile", buf.String())
