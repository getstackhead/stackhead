...
func (m DockerContainerModule) Deploy() error {
	...
	fileContents, err := xfs.ReadFile("pkging:///templates/modules/container/docker/project.tf.tmpl")
	if err != nil {
		return fmt.Errorf("Unable to open Docker template: " + err.Error())
	}
	...
	tmpl, err := template.New("docker").Funcs(funcMap).Funcs(sprig.TxtFuncMap()).Parse(fileContents)
	if err != nil {
		return err
	}
	...
	err = xfs.WriteFile("ssh://"+project.GetTerraformDirectoryPath()+"/docker.tf", buf.String())
	return err
}
